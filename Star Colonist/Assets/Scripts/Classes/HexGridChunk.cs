...
public class HexGridChunk : MonoBehaviour
{
    ...
    void TriangulateConnection(HexDirection direction, HexCell cell, EdgeVertices e1)
    {
        ...
        if (neighbor == null) //gets rid of bridges outside map
        {
            return;
        }
        ...
        {
            Vector3 v5 = e1.v5 + HexMetrics.GetBridge(direction.Next()); //sloping triangles
    ...
    {
        float boundaryPoint = 1f / (rightCell.Elevation - beginCell.Elevation);
        ...
        Vector3 boundary = Vector3.Lerp(HexMetrics.Perturb(begin), HexMetrics.Perturb(right), boundaryPoint);
        Color boundaryWeights = Color.Lerp(weights1, weights2, boundaryPoint);
        Vector3 indices;
        indices.x = beginCell.Index;
        indices.y = leftCell.Index;
        indices.z = rightCell.Index;
        ...
        {
            TriangulateBoundaryTriangle(left, weights1, right, weights2, boundary, boundaryWeights, indices);
        }
        else
        {
    ...
    {
        float boundaryPoint = 1f / (leftCell.Elevation - beginCell.Elevation);
        ...
        Vector3 boundary = Vector3.Lerp(HexMetrics.Perturb(begin), HexMetrics.Perturb(left), boundaryPoint);
        Color boundaryWeights = Color.Lerp(weights1, weights2, boundaryPoint);
        Vector3 indices;
        indices.x = beginCell.Index;
        indices.y = leftCell.Index;
    ...
    void TriangulateWithoutRiver(HexDirection direction, HexCell cell, Vector3 center, EdgeVertices e)
    {
        ...
        if(cell.HasRoads)
        {
            Vector2 tops = GetRoadtops(direction, cell);
            TriangulateRoad(center, Vector3.Lerp(center, e.v1, tops.x), Vector3.Lerp(center, e.v5, tops.y), e, cell.HasRoadThroughEdge(direction));
        }
    }
    ...
    Vector2 GetRoadtops(HexDirection direction, HexCell cell)
    {
        Vector2 tops;
        ...
        {
            tops.x = cell.HasRoadThroughEdge(direction.Previous()) ? 0.5f : 0.25f;
            tops.y = cell.HasRoadThroughEdge(direction.Next()) ? 0.5f : 0.25f;
        }
    ...
    void TriangulateRoadAdjacentToRiver(HexDirection direction, HexCell cell, Vector3 center, EdgeVertices e)
    {
        ...
        Vector3 mL = Vector3.Lerp(roadCenter, e.v1, tops.x);
        Vector3 mR = Vector3.Lerp(roadCenter, e.v5, tops.y);
    ...
    void TriangulateWater(HexDirection direction, HexCell cell, Vector3 center)
    {
        ...
        HexCell neighbor = cell.GetNeighbor(direction);
        if (neighbor != null && !neighbor.IsUnderwater) TriangulateWaterShore(direction, cell, neighbor, center);
    ...
    {
        Vector3 w1 = center + HexMetrics.GetFirstWaterCorner(direction);
        Vector3 w2 = center + HexMetrics.GetSecondWaterCorner(direction);
        water.AddTriangle(center, w1, w2);

        if (direction <= HexDirection.SE && neighbor != null)
        {
            Vector3 bridge = HexMetrics.GetWaterBridge(direction);
            Vector3 e1 = w1 + bridge;
            Vector3 e2 = w2 + bridge;
            water.AddQuad(w1, w2, e1, e2);

            if (direction <= HexDirection.E)
            {
                HexCell nextNeighbor = cell.GetNeighbor(direction.Next());
                if (nextNeighbor == null || !nextNeighbor.IsUnderwater) return;
                water.AddTriangle(w2, e2, w2 + HexMetrics.GetWaterBridge(direction.Next()));
            }
        }
    }
    ...
    void TriangulateWaterShore(HexDirection direction, HexCell cell, HexCell neighbor, Vector3 center)
    {
        ...
        {
            Vector3 v3 = 
                nextNeighbor.Position + (nextNeighbor.IsUnderwater ? 
    ...
    void TriangulateWaterfallInWater(Vector3 v1, Vector3 v2, Vector3 v3, Vector3 v4, float y1, float y2, float waterY)
    {
        ...
        float top = (waterY - y2) / (y1 - y2);
        v3 = Vector3.Lerp(v3, v1, top);
        v4 = Vector3.Lerp(v4, v2, top);
